...
def srtf_scheduler(processes):
    ...
    while completed < n:
        ...
        if current_process is not None and ready_queue:
            ready_queue.sort(key=lambda x: (x.remaining, x.arrival, x.order))
            if (ready_queue[0].remaining, ready_queue[0].arrival, ready_queue[0].order) < \
               (current_process.remaining, current_process.arrival, current_process.order):
